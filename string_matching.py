...
def calculate_overall_score(vendor_score, product_name_score, product_family_score, version_score, keyword_score,
                            vendor_threshold, product_family_threshold, product_name_threshold, keyword_threshold,
                            version_threshold):
    ...
    # Check if vendor score meets threshold
        ...
        if product_family_score is None:
            ...
            # Check if product name score is within a certain range and version and keyword scores exist
                    ...
                    product_name_threshold - 20) <= product_name_score < product_name_threshold and version_score is not None and keyword_score is not None:
...
def calculate_similarities(df1, df2, vendor_threshold, product_family_threshold, product_name_threshold,
                           keyword_threshold, version_threshold):
    ...
    for index1, row1 in df1.iterrows():
        for index2, row2 in df2.iterrows():
            ...
            if vendor_score is None:
                ...
                product_name_score = match_product(row1['product_name_modified'], row2['product_name_modified'], 1, 1,
                                                   2, 1)
                product_family_score = match_product(row1['product_family_modified'], row2['product_family_modified'])
                version_score = match_version(row1['product_version_modified'], row2['product_version_modified'],
                                              row1['product_version_range_modified'])
                keyword_score = match_keyword(row1['function_keywords_found'], row2['function_keywords_found'])

                # Calculcate Overall Score
            match, reason = calculate_overall_score(vendor_score, product_name_score, product_family_score,
                                                    version_score, keyword_score, vendor_threshold,
                                                    product_family_threshold, product_name_threshold, keyword_threshold,
                                                    version_threshold)
